...
Buffer.prototype.readALawData = function(offset, a = 87.6) {
    ...
    const two = (y) => Math.sign(y) * Math.pow(Math.E, Math.abs(y) * compression_log - 1) / [a + (a * Math.log(a))];
...
function getAmplitudeData(options) {
    ...
    const runtime = total_samples / sample_rate;

    let offset = starting_offset;
    const amplitude_data = [...new Array(channels)].map(() => new Array(total_samples));

    // const amplitude_data = [ new Array(total_samples), new Array(total_samples)];
    ...
    console.log(total_samples.toLocaleString("en-US"), runtime);
    console.log('FINAL OFFSET:', offset);
    return amplitude_data[1].length === 0 ? [amplitude_data[0]] : amplitude_data;
}

export default getAmplitudeData;